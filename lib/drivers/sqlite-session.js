...
function buildExclusionClause(db, table, exclude) {
  ...
  const cols = new Set();
  try {
    for (const row of db.prepare(`PRAGMA table_info(${ident(table)})`).all()) cols.add(row.name);
  } catch {
    return out;
  }
    ...
    const messageRows = db
      .prepare(
        `SELECT ${idCol} AS mid, ${ident(m.createdCol ?? "time_created")} AS created,
           ...
           FROM ${msgTable}
          WHERE ${sidCol} = ?
          ORDER BY ${ident(m.createdCol ?? "time_created")}, ${idCol}`,
      )
      ...
      const partRows = db
        .prepare(
          `SELECT ${ident(p.messageIdCol ?? "message_id")} AS mid,
             ...
             FROM ${ident(p.table ?? "part")}
            WHERE ${ident(p.sessionIdCol ?? "session_id")} = ?
            ORDER BY ${ident(p.createdCol ?? "time_created")}, ${ident(p.messageIdCol ?? "message_id")}`,
        )
        .all(summary.externalId);
