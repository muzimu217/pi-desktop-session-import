...
// context and tooling state, so titles and user messages must skip them.
// The list is evidence-driven from real archives (PI-Desktop #265) and kept
// in parity with the host importer (electron/main/importers/codex.ts): real
// user messages can legitimately start with "# " (pasted markdown), so the
// match stays on exact evidenced prefixes instead of a blanket "#" rule.
...
module.exports = {
  isSyntheticUserText,
  SYNTHETIC_USER_PREFIXES, source: SOURCE, label: "Codex", sessionsDirFor, scan, scanFast, convert };
